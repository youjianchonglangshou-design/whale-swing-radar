...
async function snapshot(env){
 ...
 const old=JSON.parse(await env.WHALE_DATA.get('latest')||'{}'),peaks=old.peaks||{};for(const p of positions)peaks[p.key]=Math.max(Number(peaks[p.key]||0),p.value);
 const data={wallet:WALLET,updatedAt:Date.now(),positions,fills:fills.slice(0,2000),peaks};await env.WHALE_DATA.put('latest',JSON.stringify(data));return data;
}
export default{
 async fetch(request,env){if(request.method==='OPTIONS')return new Response(null,{headers:cors});const u=new URL(request.url);try{let data;if(u.searchParams.get('refresh')==='1')data=await snapshot(env);else{const raw=await env.WHALE_DATA.get('latest');data=raw?JSON.parse(raw):await snapshot(env)}return new Response(JSON.stringify(data),{headers:cors})}catch(e){return new Response(JSON.stringify({error:e.message}),{status:500,headers:cors})}},
